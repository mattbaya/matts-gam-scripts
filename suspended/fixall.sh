...
drive_id=$1
owner=gam-admin-address@domain.com

# Add gam-admin-address as a user;
echo "Adding user gam-admin-address to the shared drive id $drive_id"
$GAM user gam-admin-address@domain.com add drivefileacl $drive_id user gam-admin-address@domain.com role editor asadmin 2>/dev/null

# Query the files in the shared drive and output only the files with "(PENDING DELETION - CONTACT OIT)" in the name
allfiles="$( $GAM user gam-admin-address@domain.com show filelist select teamdriveid "$drive_id" fields "id,name" )"

#echo "$allfiles"
#echo "---------"
files=$(echo "$allfiles"|egrep -v "Owner,id")
#echo "$files"
echo "---------"
...
while IFS=, read -r owner fileid filename; do
  ...
  if [[ "$new_filename" != "$filename" ]]; then
    ...
    echo "Renamed file: $filename -> $new_filename" >> logs/$drive_id-renames.txt
  fi
...
/root/bin/gamadv-x/gam user gam-admin-address@domain.com delete drivefileacl $drive_id gam-admin-address@domain.com asadmin 2>/dev/null
echo "Log file for this is at logs/$drive_id-renames.txt"
echo "--------------------------------------------------"
