...
/root/bin/gamadv-x/gam user "$user" show filelist id name | grep "(PENDING DELETION - CONTACT OIT)" > /opt/organization/mjb9/suspended/tmp/gam_output_$user.txt
...
((counter++))
  ...
  if [[ "$new_filename" != "$filename" ]]; then
    ...
    echo "Renamed file: $fileid, $filename -> $new_filename (Suspended Account - Temporary Hold)" >>  /opt/organization/mjb9/suspended/tmp/$user-fixed.txt
